...
- `.createOrReuse(className[, onCreate])`: behave the same way as `create()` when the object doesn't exist already, but will just do nothing (except capturing the log line by default) when the object already exists
